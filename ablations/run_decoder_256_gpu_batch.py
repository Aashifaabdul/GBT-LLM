"""Round-trip check of a 256x256 center crop: encode to a .bin container, decode it from disk, compare.
...
identical to the encoder's, and prints bpp, Y/RGB-PSNR and Y-SSIM. Reads
data/<sequence>/frames/frame0000.png and checkpoints/stageA.pt (falls back to the untrained
ContextGradientGBTICL if absent); writes the crop, bitstream and both reconstructions to
results/decoder_256_gpu_verification/. The container layout is described in encode_to_bitstream.
...
MAGIC_HEADER = b"GBTB256\x01"


def center_crop_snapped(img, size=256, block_size=8):
    """Crop a centered size x size region whose offsets and size are multiples of block_size."""
    h, w = img.shape[:2]
...
def encode_to_bitstream(ycbcr_np, gbticl_model, quant_step=8.0,
                         symbol_range=(-2200, 2200), block_size=8, device="cuda"):
    """Encode a YCbCr frame into a self-contained container.

    Returns:
        container_bytes: header + histogram + range-coder payload.
        canvas_enc: encoder-side reconstruction, shape (H, W, 3).
        enc_q: quantized coefficients, shape (n_bh, n_bw, 64, 3).
        enc_time: encoding time in seconds.
    """
    ...
    # Transform and quantize every block, reconstructing as the decoder will so the causal context matches.
    for i in range(n_bh):
        for j in range(n_bw):
    ...
    # Static model: frame histogram; unused symbols below the maximum get a small floor so no probability is zero.
    ...
    # Container (big-endian): magic (8 B), H u16, W u16, block_size u8, quant_step f32,
    # symbol_range_min i32, symbol_range_max i32, max_sym i32, probs_len u32,
    # then the float64 histogram and the range-coder words. The histogram counts towards the size.
...
@torch.inference_mode()
def decode_from_bitstream_file(bitstream_path, gbticl_model, device="cuda"):
    """Reconstruct an image from a .bin container; no encoder state or original pixels are used.

    Returns:
        canvas_dec: reconstruction, shape (H, W, 3).
        dec_q: decoded quantized coefficients, shape (n_bh, n_bw, 64, 3).
        dec_time: decoding time in seconds.
        meta: header fields and sizes.
    """
    device = torch.device(device)
    ...
    cat_model = constriction.stream.model.Categorical(probs, perfect=False)
    n_bh, n_bw = H // block_size, W // block_size
    total_coeffs = n_bh * n_bw * block_size * block_size * 3

    # All symbols are decoded first; the graph bases are then rebuilt block by block from decoded pixels.
...
def run_verification(sequence="Beauty", crop_size=256, quant_step=8.0, device="cuda"):
    """Encode, decode and compare one crop, print a report and return the key metrics as a dict."""
    print("=" * 80)
    print(" 256x256 GPU DECODER BITSTREAM RECONSTRUCTION & BIT-EXACT VERIFICATION")
    print("=" * 80)

    device = torch.device(device if torch.cuda.is_available() else "cpu")
    print(f"  Device:               {device} ({torch.cuda.get_device_name(0) if device.type == 'cuda' else 'CPU'})")
    print(f"  Sequence:             {sequence}")
    print(f"  Resolution:           {crop_size} x {crop_size} (1,024 blocks of 8x8)")
    print(f"  Quantization Step:    {quant_step}")
    print("  Entropy Engine:       Compiled C++/Rust Constriction Range Coder")

    img_path = ROOT_DIR / f"data/{sequence}/frames/frame0000.png"
    if not img_path.exists():
        raise FileNotFoundError(f"Input image not found: {img_path}")

    raw_img = Image.open(img_path).convert("RGB")
    orig_rgb = center_crop_snapped(np.array(raw_img), crop_size)
    orig_ycbcr = rgb_to_ycbcr(orig_rgb)

    out_dir = ROOT_DIR / "results/decoder_256_gpu_verification"
    out_dir.mkdir(parents=True, exist_ok=True)
    orig_png_path = out_dir / f"original_{sequence}_{crop_size}x{crop_size}.png"
    Image.fromarray(orig_rgb).save(orig_png_path)
    ...
    print("\n[Step 1/3] Running GPU Encoder & Writing .bin bitstream file...")
    container_bytes, enc_canvas, enc_q, enc_time = encode_to_bitstream(
        orig_ycbcr, gbticl_model, quant_step=quant_step,
        symbol_range=(-2200, 2200), block_size=8, device=device
    )

    bin_path = out_dir / f"bitstream_{sequence}_{crop_size}x{crop_size}_q{int(quant_step)}.bin"
    with open(bin_path, "wb") as f:
        f.write(container_bytes)

    enc_rgb = ycbcr_to_rgb(enc_canvas.detach().cpu().numpy())
    enc_png_path = out_dir / f"encoder_recon_{sequence}_{crop_size}x{crop_size}.png"
    Image.fromarray(enc_rgb).save(enc_png_path)

    bpp = (len(container_bytes) * 8.0) / (crop_size * crop_size)
    print(f"  -> Encoded in:        {enc_time*1000:.2f} ms")
    print(f"  -> Container Size:    {len(container_bytes):,} bytes ({bpp:.4f} bpp)")
    print(f"  -> Saved Bitstream:   {bin_path.name}")
    print(f"  -> Encoder Recon:     {enc_png_path.name}")

    print("\n[Step 2/3] Running Independent GPU Decoder from .bin file on disk...")
    dec_canvas, dec_q, dec_time, meta = decode_from_bitstream_file(
        bin_path, gbticl_model, device=device
    )

    dec_rgb = ycbcr_to_rgb(dec_canvas.detach().cpu().numpy())
    dec_png_path = out_dir / f"decoder_recon_{sequence}_{crop_size}x{crop_size}.png"
    Image.fromarray(dec_rgb).save(dec_png_path)

    print(f"  -> Decoded in:        {dec_time*1000:.2f} ms")
    print(f"  -> Saved Dec Recon:   {dec_png_path.name}")

    # Any mismatch means the decoder's causal context diverged from the encoder's.
    ...
    mean_pixel_diff = float(np.mean(pixel_abs_diff))
    ...
    print(f"    - Mean Absolute Pixel Error:      {mean_pixel_diff:.6f}")
    ...
    if lossless_symbols and max_pixel_diff == 0.0:
        print("[SUCCESS] The decoder-side reconstruction from .bin is 100% BIT-IDENTICAL to the encoder!")
    else:
        print("[WARNING] Discrepancy detected between encoder and decoder reconstructions!")
    print("=" * 80)

    return {
        "lossless_symbols": lossless_symbols,
        ...
        "bpp": bpp,
        "y_psnr": dec_y_psnr,
        "rgb_psnr": dec_rgb_psnr,
        "y_ssim": dec_y_ssim
    }


def main():
    parser = argparse.ArgumentParser(description="256x256 GPU decoder reconstruction and verification.")
    parser.add_argument("--sequence", type=str, default="Beauty", choices=["Beauty", "HoneyBee"], help="Sequence name")
    parser.add_argument("--crop", type=int, default=256, help="Crop size (default: 256)")
    ...
    run_verification(sequence=args.sequence, crop_size=args.crop, quant_step=args.quant_step, device=args.device)
